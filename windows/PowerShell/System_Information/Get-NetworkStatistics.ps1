...
<#
This script displays the current TCP/IP connections for a local or remote system to include the PID, 
process name, port, and its current state (listening, etc..). If the port is not yet established, the 
port number is shown as an asterisk (*) It will also take the initial output and save it to old_state.txt 
and then sleep for a period of time of your choosing before running again and outputting to new_state.txt. 
It will then compare the two files and print the output to the screen. Both files will be saved to the 
directory in which the script was ran from (not located). It will continue to do this process until the
script is stopped.
    ...
    .NOTES
	    Portions of this code was taken from http://ramblingcookiemonster.github.io/
...
# Runs once and outputs to old_state.txt
Get-networkstatistics | Format-Table > .\old_state.txt
while($true)
{
    # Sleep time by seconds
    Start-Sleep -seconds 3
    Get-NetworkStatistics | Format-Table > .\new_state.txt
    fc.exe /U ".\old_state.txt" ".\new_state.txt"
}